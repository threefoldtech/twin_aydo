...
const authState = reactive<AuthState>({
    user: {
        ...
        image: `${config.baseUrl}/api/v2/user/avatar/default`,
        email: 'testemail',
        status: '',
        location: '',
    },
...
export const myYggdrasilAddress = async () => {
    const res = await axios.get(`${config.baseUrl}/api/v2/locations/yggdrasil`);
    return res.data;
};

export const getMe = async (): Promise<{ username: string; email: string }> => {
    const res = await axios.get(`${config.baseUrl}api/v2/user/me`);
    return {
